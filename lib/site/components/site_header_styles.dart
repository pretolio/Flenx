...
.site-header{display:block;position:sticky;top:0;z-index:50;background:#fff;border-bottom:1px solid var(--border,#E2E8F0);font-family:system-ui,-apple-system,sans-serif}
.header-inner{max-width:1120px;margin:0 auto;padding:0 20px;height:64px;display:flex;align-items:center;gap:20px}
.brand{flex-shrink:0;display:flex;align-items:center;gap:8px;text-decoration:none;color:var(--primary);font-weight:800;font-size:1.25rem}
.brand-img{height:32px}
.nav-toggle{display:none}
.hamburger{display:none;cursor:pointer;font-size:1.6rem;line-height:1;color:var(--primary);padding:6px;user-select:none}
...
.dropdown{position:absolute;top:100%;left:0;min-width:210px;background:#fff;border:1px solid var(--border,#E2E8F0);border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,.08);list-style:none;margin:6px 0 0;padding:6px;opacity:0;visibility:hidden;transform:translateY(6px);transition:.15s ease}
...
.dropdown li a:hover{background:#f3f4f6;color:var(--primary)}
...
@media (max-width:860px){
 ...
 .nav-wrap{display:flex;position:fixed;top:64px;right:0;bottom:0;left:auto;width:min(80vw,320px);flex-direction:column;align-items:stretch;gap:0;background:#fff;border-left:1px solid var(--border,#E2E8F0);padding:14px;box-shadow:-12px 0 32px rgba(0,0,0,.12);margin-left:0;overflow-y:auto;transform:translateX(100%);transition:transform .25s ease;z-index:56}
