...
class FlenxPdf {
    ...
    for (final page in doc.pages) {
      switch (page) {
        ...
        case FlenxPdfChecklist():
          pdf.addPage(_sectionPage(b, page.tone, _checklist(b, page)));
        case FlenxPdfText():
          pdf.addPage(_sectionPage(b, page.tone, _text(b, page)));
        case FlenxPdfSpotlight():
          final im = await image(page.imagePath);
          pdf.addPage(_sectionPage(b, page.tone, _spotlight(b, page, im)));
        case FlenxPdfSteps():
          pdf.addPage(_sectionPage(b, page.tone, _steps(b, page)));
        case FlenxPdfContact():
          final im = await image(b.logoDarkBgPath);
          pdf.addPage(_sectionPage(b, page.tone, _contact(b, page, im)));
      }
    }
    return pdf.save();
  }
  ...
  // ---------- páginas ----------
  static pw.Page _sectionPage(FlenxPdfBrand b, FlenxPdfTone tone, pw.Widget child) {
    return pw.Page(
  ...
  static pw.Widget _head(FlenxPdfBrand b, FlenxPdfTone tone, String? eyebrow, String title, [String? subtitle, bool center = true]) {
    return pw.Column(crossAxisAlignment: center ? pw.CrossAxisAlignment.center : pw.CrossAxisAlignment.start, children: [
      ...
      pw.Text(title, textAlign: center ? pw.TextAlign.center : pw.TextAlign.left,
          style: pw.TextStyle(color: _title(b, tone), fontWeight: pw.FontWeight.bold, fontSize: 22)),
      if (subtitle != null) ...[
        pw.SizedBox(height: 8),
        pw.Text(subtitle, textAlign: center ? pw.TextAlign.center : pw.TextAlign.left,
            style: pw.TextStyle(color: _body(b, tone), fontSize: 11)),
      ],
    ]);
  }
        ...
        child: pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          ...
          pw.Expanded(
            child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
              pw.Text(it.title, style: pw.TextStyle(color: _title(b, tone), fontWeight: pw.FontWeight.bold, fontSize: 11.5)),
              if (it.description != null)
                pw.Text(it.description!, style: pw.TextStyle(color: _body(b, tone), fontSize: 10, lineSpacing: 1.5)),
            ]),
          ),
        ]),
      );
  ...
  static pw.Widget _checklist(FlenxPdfBrand b, FlenxPdfChecklist p) {
    ...
    return pw.Column(mainAxisSize: pw.MainAxisSize.min, children: [
      ...
      if (p.columns >= 2 && rows.length > 3) _twoCols(rows) else pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: rows),
    ]);
  }
  ...
  static pw.Widget _text(FlenxPdfBrand b, FlenxPdfText p) {
    return pw.Column(mainAxisSize: pw.MainAxisSize.min, crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
      ...
      for (final para in p.paragraphs) ...[
        pw.Text(para, style: pw.TextStyle(color: _body(b, p.tone), fontSize: 11.5, lineSpacing: 2.2)),
        pw.SizedBox(height: 10),
      ],
      if (p.stats.isNotEmpty) ...[
        pw.SizedBox(height: 14),
        pw.Wrap(spacing: 40, runSpacing: 16, children: [
          for (final s in p.stats)
            pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
              ...
              pw.Text(s.label, style: pw.TextStyle(color: _body(b, p.tone), fontSize: 9.5)),
            ]),
        ]),
      ],
    ]);
  ...
  static pw.Widget _bullet(FlenxPdfBrand b, FlenxPdfTone tone, String t) => pw.Padding(
        ...
        child: pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          pw.Container(width: 16, height: 16, margin: const pw.EdgeInsets.only(top: 1, right: 8),
              ...
              alignment: pw.Alignment.center,
              child: _check()),
          pw.Expanded(child: pw.Text(t, style: pw.TextStyle(color: _title(b, tone), fontSize: 11, lineSpacing: 1.6))),
        ]),
      );
  ...
  static pw.Widget _spotlight(FlenxPdfBrand b, FlenxPdfSpotlight p, pw.MemoryImage? im) {
    return pw.Column(mainAxisSize: pw.MainAxisSize.min, crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
      ...
      if (p.description != null) ...[
        pw.SizedBox(height: 10),
        pw.Text(p.description!, style: pw.TextStyle(color: _body(b, p.tone), fontSize: 11.5, lineSpacing: 2)),
      ],
      pw.SizedBox(height: 14),
      for (final t in p.bullets) _bullet(b, p.tone, t),
      if (im != null) ...[
        pw.SizedBox(height: 18),
        pw.ClipRRect(horizontalRadius: 10, verticalRadius: 10, child: pw.Image(im, width: double.infinity)),
      ],
    ]);
  }
  ...
  static pw.Widget _steps(FlenxPdfBrand b, FlenxPdfSteps p) {
    ...
    final rows = p.steps.map((s) {
      ...
      return pw.Padding(
        ...
        child: pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          pw.Container(width: 26, height: 26, margin: const pw.EdgeInsets.only(right: 12),
              decoration: pw.BoxDecoration(color: _c(b.primary), borderRadius: pw.BorderRadius.circular(7)),
              alignment: pw.Alignment.center,
              child: pw.Text('$n', style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 12))),
          pw.Expanded(child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
            pw.Text(s.title, style: pw.TextStyle(color: _title(b, p.tone), fontWeight: pw.FontWeight.bold, fontSize: 12.5)),
            if (s.description != null) pw.Text(s.description!, style: pw.TextStyle(color: _body(b, p.tone), fontSize: 10.5, lineSpacing: 1.6)),
          ])),
        ]),
      );
    }).toList();
  ...
  static pw.Widget _contact(FlenxPdfBrand b, FlenxPdfContact p, pw.MemoryImage? logo) {
    return pw.Column(mainAxisSize: pw.MainAxisSize.min, crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
      if (logo != null) ...[pw.Image(logo, width: 190), pw.SizedBox(height: 22)],
      pw.Text(p.title, textAlign: pw.TextAlign.center, style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 22)),
      if (p.subtitle != null) ...[
        pw.SizedBox(height: 10),
        pw.Text(p.subtitle!, textAlign: pw.TextAlign.center, style: pw.TextStyle(color: _c('#c3d2ec'), fontSize: 11)),
      ],
      pw.SizedBox(height: 18),
      for (final l in p.lines)
        pw.Padding(padding: const pw.EdgeInsets.only(bottom: 5),
            child: pw.Text(l, textAlign: pw.TextAlign.center, style: pw.TextStyle(color: PdfColors.white, fontSize: 11))),
    ]);
  }
