...
class FirestoreDbExecutor implements DbExecutor {
  ...
  factory FirestoreDbExecutor.fromEnv(Map<String, String> env,
          {http.Client? client}) =>
      FirestoreDbExecutor(
  ...
  Future<int> insert(String table, Map<String, Object?> values) async {
    final r = await _client.post(Uri.parse('$_docs/$table'),
        headers: _headers, body: jsonEncode({'fields': _fields(values)}));
    if (r.statusCode >= 400) {
  ...
  Future<List<Map<String, Object?>>> list(
    ...
    final r = await _client.post(Uri.parse('$_docs:runQuery'),
        headers: _headers, body: jsonEncode(query));
    if (r.statusCode >= 400) {
  ...
  Future<int> count(String table) async {
    ...
    final r = await _client.post(Uri.parse('$_docs:runAggregationQuery'),
        headers: _headers, body: jsonEncode(query));
    if (r.statusCode >= 400) return 0;
    final res = (jsonDecode(r.body) as List).first as Map;
    final agg = ((res['result'] as Map)['aggregateFields'] as Map)['count'] as Map;
    return int.tryParse('${agg['integerValue']}') ?? 0;
  }

  @override
  Future<Map<String, Object?>?> findById(String table, Object id) async {
    final r = await _client.get(Uri.parse('$_docs/$table/$id'), headers: _headers);
  ...
  Future<bool> updateById(
    ...
    final mask =
        values.keys.map((k) => 'updateMask.fieldPaths=$k').join('&');
  ...
  Future<bool> deleteById(String table, Object id) async {
    final r =
        await _client.delete(Uri.parse('$_docs/$table/$id'), headers: _headers);
    return r.statusCode < 400;
  }
}
