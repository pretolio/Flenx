...
Future<Map<String, dynamic>> _post(Handler h, String path, String body) async {
  final res = await h(Request('POST', Uri.parse('http://x$path'),
      body: body,
      headers: {'content-type': 'application/x-www-form-urlencoded'}));
  return {'status': res.statusCode, 'json': jsonDecode(await res.readAsString())};
}

void main() {
  group('FlextApi (runtime Dart)', () {
    test('insere e responde no envelope padrão', () async {
      final h = _router();
      final r = await _post(h, '/api/leads', 'name=Ana&email=ana@x.com&message=oi');
      expect(r['status'], 200);
    ...
    test('listagem é paginada com meta', () async {
      ...
      final res = await h(Request('GET',
          Uri.parse('http://x/api/leads/list?page=2&perPage=10')));
