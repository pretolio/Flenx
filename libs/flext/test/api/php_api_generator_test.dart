...
import 'package:flext/db/db.dart';
...
void main() {
  group('PhpApiGenerator', () {
    ...
    test('gera o bootstrap _flext.php (PDO, envelope, .env)', () {
      final boot = files['_flext.php']!;
      expect(boot, contains('function flext_db()'));
      expect(boot, contains('new PDO('));
      expect(boot, contains("flext_env('DB_PASSWORD'"));
      expect(boot, contains('function flext_respond('));
      expect(boot, contains('function flext_page('));
    });

    test('gera leads.php com validação, INSERT preparado e redirect', () {
      final php = files['leads.php']!;
      expect(php, contains("require __DIR__ . '/_flext.php'"));
