    ...
    switch (block.type) {
      ...
      case BlockType.image:
        return [
          ...
          _field('Crédito (ex.: Foto: Agência)',
              (v) => block.secondary = v, block.secondary),
        ];
      case BlockType.quote:
        return [
          _field('Texto da citação', (v) => block.text = v, block.text,
              lines: 2),
      ...
      case BlockType.list:
        return [
          _field('Itens (um por linha)', (v) => block.text = v, block.text,
              lines: 3),
          Row(children: [
            Checkbox(
                value: block.ordered,
      ...
      case BlockType.embed:
        return [
          _field('HTML do embed (<iframe ...>)', (v) => block.text = v,
              block.text,
              lines: 3),
        ];
      case BlockType.paragraph:
        return [
          _field('Texto (aceita Markdown)', (v) => block.text = v, block.text,
              lines: 3),
        ];
    }
  }

  Widget _field(String label, ValueChanged<String> onUpdate, String initial,
      {int lines = 1}) {
    return Padding(
