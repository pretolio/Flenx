...
void main() {
  ...
  group('RestApiDbExecutor (PHP ou Dart)', () {
    test('insert lê data.id do envelope', () async {
      ...
      final db = RestApiDbExecutor(
        baseUrl: 'https://api.x/api',
        client: client,
      );
  ...
  group('DbRegistry.fromEnv', () {
    test('escolhe o backend por DB_PROVIDER', () {
      ...
      expect(
        DbRegistry.fromEnv({'DB_PROVIDER': 'rest'}),
        isA<RestApiDbExecutor>(),
      );
      expect(
        DbRegistry.fromEnv({'DB_PROVIDER': 'firebase'}),
    ...
    test('provider inválido lança erro', () {
      expect(
        () => DbRegistry.fromEnv({'DB_PROVIDER': 'oracle'}),
        throwsArgumentError,
      );
    });
  });
