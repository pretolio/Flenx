...
/// Modelos do banco. `blogPostsModel`, `categoriesModel`, `usersModel` e
/// `settingsModel` vêm da lib e geram as tabelas (notícias salvas pelo admin,
/// categorias curadas, usuários e a config da home num único registro JSON).
const dbModels = [blogPostsModel, categoriesModel, usersModel, settingsModel];
...
const apis = [
  ...
  // ---- Categorias: CRUD (lista curada de editorias) ----
  ApiEndpoint(
    path: '/api/categories',
    method: HttpMethod.post,
    fields: [
    ...
    actions: [InsertInto(categoriesModel)],
  ),
  ApiEndpoint(
    path: '/api/categories/list',
    method: HttpMethod.get,
    actions: [ListPaginated(categoriesModel)],
  ),
  ApiEndpoint(
    path: '/api/categories/update',
    method: HttpMethod.post,
    fields: [Field('id', required: true)],
    actions: [UpdateById(categoriesModel)],
  ),
  ApiEndpoint(
    path: '/api/categories/delete',
    method: HttpMethod.post,
    fields: [Field('id', required: true)],
    actions: [DeleteById(categoriesModel)],
  ),
